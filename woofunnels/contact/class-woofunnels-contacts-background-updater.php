...
class WooFunnels_Contacts_Background_Updater extends WP_Background_Process {
	...
	public function handle_cron_healthcheck() {
		...
		if ( $this->is_queue_empty() ) {
			...
			WooFunnels_Dashboard::$classes['BWF_Logger']->log( 'Scheduled event cleared as queue is empty.', 'woofunnels_contacts_indexing' );
		...
		if ( self::MAX_SAME_OFFSET_THRESHOLD === count( $offsets ) ) {
			...
			if ( 1 === count( $unique ) ) {
				$this->kill_process();
				WooFunnels_Dashboard::$classes['BWF_Logger']->log( sprintf( 'Offset is stuck from last %d cron jobs, terminating the process.', self::MAX_SAME_OFFSET_THRESHOLD ), 'woofunnels_contacts_indexing' );
		...
		WooFunnels_Dashboard::$classes['BWF_Logger']->log( 'Cron started again!!', 'woofunnels_contacts_indexing' );
	...
	public function maybe_re_dispatch_background_process() {
		...
		if ( self::MAX_SAME_OFFSET_THRESHOLD === count( $offsets ) ) {
			...
			if ( 1 === count( $unique ) ) {
				$this->kill_process();
				WooFunnels_Dashboard::$classes['BWF_Logger']->log( sprintf( 'Offset is stuck from last %d attempts, terminating the process.', self::MAX_SAME_OFFSET_THRESHOLD ), 'woofunnels_contacts_indexing' );
	...
	protected function task( $callback ) {
		...
		if ( is_callable( $callback ) ) {
			WooFunnels_Dashboard::$classes['BWF_Logger']->log( 'Running the callback: ' . print_r( $callback, true ), 'woofunnels_contacts_indexing' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			$result = (bool) call_user_func( $callback );

			if ( $result ) {
				/**sleep( 5 );*/
				WooFunnels_Dashboard::$classes['BWF_Logger']->log( "Result: $result Need to run again the callback: " . print_r( $callback, true ), 'woofunnels_contacts_indexing' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			} else {
				WooFunnels_Dashboard::$classes['BWF_Logger']->log( "Result: $result Finished running the callback: " . print_r( $callback, true ), 'woofunnels_contacts_indexing' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			}
		} else {
			WooFunnels_Dashboard::$classes['BWF_Logger']->log( "Result: $result Could not find the callback: " . print_r( $callback, true ), 'woofunnels_contacts_indexing' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		}
	...
	protected function complete() {
		...
		WooFunnels_Dashboard::$classes['BWF_Logger']->log( 'Background scanning completed for indexing contacts and creating updating contacts and customers.', 'woofunnels_contacts_indexing' );
